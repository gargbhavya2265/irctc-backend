...
class EmailConsumer {
  async start() {
    try {
      ...
      logger.info(
        'Email consumer is running and listening for messages...'
      );
    } catch (error) {
  ...
  async handleMessage(topic, eventType, data) {
    switch (eventType) {
      ...
      default:
        logger.info(
          `Ignoring event: ${eventType} from topic: ${topic}`
        );
    }
  }
  ...
  async handleBookingConfirmed(data) {
    ...
    if (!email) {
      logger.warn(
        'Skipping booking-confirmed email — no email on event',
        { bookingId }
      );
      return;
    }
    ...
    logger.info(`Booking confirmed email sent to ${email}`, {
      bookingId,
    });
  }
  ...
  async handleBookingFailed(data) {
    ...
    if (!email) {
      logger.warn(
        'Skipping booking-failed email — no email on event',
        { bookingId }
      );
      return;
    }
    ...
    logger.info(`Booking failed email sent to ${email}`, {
      bookingId,
    });
  }
  ...
  async handleBookingCancelled(data) {
    ...
    if (!email) {
      logger.warn(
        'Skipping booking-cancelled email — no email on event',
        { bookingId }
      );
      return;
    }
    ...
    logger.info(`Booking cancelled email sent to ${email}`, {
      bookingId,
    });
  }
