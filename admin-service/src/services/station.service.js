...
/**
 * Reindex all stations into Elasticsearch through Kafka.
 *
 * Existing stations may already exist in PostgreSQL but their
 * STATION_CREATED events may have been missed by Search Service.
 *
 * This publishes STATION_CREATED events again for every station.
 */
const reindexAllStations = async () => {
  ...
  for (const station of stations) {
    try {
      ...
      logger.info(
        `Reindex event published for station ${station.name} (${station.code})`
      );
    } catch (err) {
      logger.error(
        `Failed to publish reindex event for ${station.name} (${station.code})`,
        {
          error: err.message
        }
      );
